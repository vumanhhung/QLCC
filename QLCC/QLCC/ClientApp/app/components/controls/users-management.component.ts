...
export class UsersManagementComponent implements OnInit, AfterViewInit {
    ...
    @ViewChild('nguoidungtoanhaEditor')
    nguoidungtoanhaEditor: NguoiDungToaNhaInfoComponent;
    ...
    ngOnInit() {
        ...
        if (this.canManageUsers)
            this.columns.push({ name: gT('matbang.qlmb_chucnang'), width: 100, cellTemplate: this.actionsTemplate, resizeable: false, canAutoResize: false, sortable: false, draggable: false, cellClass: "overflow" });
    ...
    roleUser(row: UserEdit) {            
        this.nguoidungtoanhaEditor.editNguoiDungToaNha(row);
        this.nguoidungtoanhaEditor.editorModal.show();
    }

    deleteUser(row: UserEdit) {
        this.alertService.showDialog('Bạn có chắc chắn muốn xóa \"' + row.userName + '\"?', DialogType.confirm, () => this.deleteUserHelper(row));
    }
    ...
    deleteUserHelper(row: UserEdit) {
        ...
        this.accountService.deleteUser(row)
            ...
            error => {
                ...
                this.alertService.showStickyMessage("Xóa lỗi", `Đã xảy ra lỗi khi xóa người dùng.\r\nLỗi: "${Utilities.getHttpResponseMessage(error)}"`,
                    MessageSeverity.error, error);
            });
    }
