...
export class NotificationsViewerComponent implements OnInit, OnDestroy {
    ...
    initDataLoading() {
        ...
        this.dataLoadingSubscription = dataLoadTask
            ...
            error => {
                ...
                this.alertService.showMessage("Tải lỗi", "Đang tải thông báo mới từ máy chủ không thành công!", MessageSeverity.warn);
                this.alertService.logError(error);

                if (this.dataLoadingConsecutiveFailurs++ < 5)
                    setTimeout(() => this.initDataLoading(), 5000);
                else
                    this.alertService.showStickyMessage("Tải lỗi", "Đang tải thông báo mới từ máy chủ không thành công!", MessageSeverity.error);
    ...
    deleteNotification(row: Notification) {
        this.alertService.showDialog('Bạn có chắc chắn muốn xóa thông báo \"' + row.header + '\"?', DialogType.confirm, () => this.deleteNotificationHelper(row));
    }
    ...
    deleteNotificationHelper(row: Notification) {
        ...
        this.notificationService.deleteNotification(row)
            ...
            error => {
                ...
                this.alertService.showStickyMessage("Lỗi xóa", `Đã xảy ra lỗi trong khi xóa thông báo.\r\nLỗi: "${Utilities.getHttpResponseMessage(error)}"`,
    ...
    togglePin(row: Notification) {
        ...
        this.notificationService.pinUnpinNotification(row, pin)
            ...
            error => {
                ...
                this.alertService.showStickyMessage(opText + " lỗi", `Đã xảy ra lỗi trong khi ${opText} thông báo.\r\nLỗi: "${Utilities.getHttpResponseMessage(error)}"`,
                    MessageSeverity.error, error);
            });
    }
